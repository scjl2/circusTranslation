...
private NewSCJApplication context;
   ...
   class NewDataOpMethodModel extends
...
//      DATA_OBJECT = FACTORY.getTemplate("DataObject.ftl");
      ...
      try
	{
		...
		FACTORY.CIRCUS_CONFIG.setSharedVariable("ACTION", new MethodBodyVisitor(newSCJApp, null));
		FACTORY.CIRCUS_CONFIG.setSharedVariable("DATAOP", new NewDataOpMethodModel(newSCJApp, null));
    	  }
   ...
   public void setObjectEnv(ObjectEnv object)
   {
	   try
	{
		FACTORY.CIRCUS_CONFIG.setSharedVariable("ACTION", new MethodBodyVisitor(context, object));
		FACTORY.CIRCUS_CONFIG.setSharedVariable("DATAOP", new NewDataOpMethodModel(context, object));
	} catch (TemplateModelException e)
	{
		// TODO Auto-generated catch block
		e.printStackTrace();
