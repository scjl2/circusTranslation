...
public class ObjectEnv
{
  ...
  /**
   * The required parents of this object. I.E. the files that this object's file
   * will depend on to parse
   */
  ...
  public void addVariable(String variableName, String variableType, Object variableInit,
      boolean primitive)
  {
  ...
  /**
   * Gets a variable environment from this object which shares the same name as
   * <code>name</code>. If the variable doesn't exist in this object
   * environment, then this method will return <code>null</code> This method use
   * <code>getVariable(String name)</code> internally
   * 
   * @param name
  ...
  public VariableEnv getParameter(String name)
  {
    ...
    for (VariableEnv v : getParameters())
    {
      ...
      {
        Debugger.log("param name = " + v.getName());
        return v;
      }
    }
...

  {
    ...
    if (isNew)
    {
      VariableEnv e = new VariableEnv(variableName, variableType, programType, primitive);
      e.setInit(init);
      addFWdParameter(e);
    }
  ...
  public List<Map> varsList()
  {
    ...
    for (VariableEnv v : variables)
    {
      ...
      varMap.put(hijac.tools.tightrope.utils.TightRopeString.ObjectString.VAR_NAME, v
          .getName().toString());
      varMap.put(hijac.tools.tightrope.utils.TightRopeString.ObjectString.VAR_TYPE,
          v.getType());
      if (v.getInit() != null)
      {
        varMap.put(hijac.tools.tightrope.utils.TightRopeString.ObjectString.VAR_INIT, v
            .getInit().toString());
      }
      varMap.put(hijac.tools.tightrope.utils.TightRopeString.ObjectString.VAR_INPUT,
          v.getInput());
  ...
  /**
   * Gets the lists of all the non-<code>synchronized</code> methods for the
   * object this Environment represented.
   * 
  ...
  /**
   * Gets the lists of all the <code>synchronized</code> methods for the object
   * this Environment represented.
   * 
  ...
  {
    VariableEnv toRemove = null;

    for (VariableEnv ve : variables)
    {
      if (ve.getName().equals(name2))
      {
        toRemove = ve;
      }
    }
